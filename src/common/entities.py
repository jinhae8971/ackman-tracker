...
class Entity:
    ...
    # --- 무결성 게이트 ---------------------------------------------------
    # 총액 전분기 대비 변동 허용치(%). None 이면 schema.MAX_TOTAL_SWING_PCT(80%).
    # 이 가드가 실제로 잡는 것은 '단위 오류'(천 단위를 달러로 읽으면 +99,900%)와
    # '테이블 절반만 파싱'이다. 성숙한 운용사에서 +80% 는 그런 버그의 신호지만,
    # 자금이 급유입되는 신생 펀드에서는 정상 성장이다. 엔티티가 스스로
    # 근거를 밝히고 올릴 수 있게 열어 둔다.
    max_total_swing_pct: Optional[float] = None
...
SITUATIONAL = Entity(
    ...
    # 총액 변동 허용치를 400% 로 올리는 근거
    # ------------------------------------
    # 실측 분기별 변동: +295% / +111% / +95% / +33% / +148%.
    # 기본값 80% 로는 6개 분기 중 4개가 걸린다. 이 펀드는 2024Q4 $0.25B 에서
    # 2026Q1 $13.7B 로 커진 신생 펀드이고, 그 성장 자체가 추적 대상이다.
    #
    # 400% 로 올려도 가드는 살아 있다. 단위 오류(천 단위 → 달러, +99,900%)는
    # 249배의 여유를 두고 잡히고, 테이블 절반 유실(-50%)도 그대로 잡힌다.
    # 행 누락은 파서의 entryTotal 체크섬이 이미 하드 실패로 막는다.
    max_total_swing_pct=400.0,
